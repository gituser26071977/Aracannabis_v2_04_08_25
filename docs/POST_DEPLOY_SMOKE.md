...
**Data:** 2026-06-25 **Modo:** EXECUTE (somente documentação) **Origem:**
corrigido conforme `RUNBOOK_VALIDATION_REPORT.md` (M22.1)

---

## Como usar

Cada item lista: comando, resultado esperado, tempo máximo, ação se falhar. Se
QUALQUER item falhar, acionar `rollback.sh --env=production` (ver
`ROLLBACK_PLAYBOOK.md`).

**REGRA DE OURO:** se o operador ver HTTP **404** num endpoint que deveria
existir, **NÃO é bug** se o endpoint estiver marcado como `[NÃO DEPLOYADO]`. É
bug se o endpoint estiver marcado como `[EXISTE]`.
...
> **Nota:** `/api/nutrologia` **NÃO EXISTE** no código. Operador **não deve
> testar este endpoint**. Documentado em M22.1.
...
> **Correção M22.1:** rota real é `/api/chat-simples` (não
> `/api/ai-chat-simples/perguntar`).
...
> - `/api/billing/history` → **`/api/billing/invoices`** (única rota de
>   histórico existente)
...
## 12. Webhook Telegram (substitui Evolution WhatsApp — D05k)

| Item                    | Comando                                                                                                                                                                                                                                      | Esperado                                                     | Tempo máx | Status deploy                            | Se falhar                    |
| ----------------------- | REDACTED | REDACTED | --------- | REDACTED | ---------------------------- |
| POST sem secret         | `curl -X POST https://api.visualsmartflow.com.br/api/dr-anderson/webhooks/telegram -d '{}'`                                                                                                                                                  | 401                                                          | 5s        | [EXISTE] `routes/dr_anderson_webhook.py` | OK se rejeitado              |
| POST com secret correto | `curl -X POST https://api.visualsmartflow.com.br/api/dr-anderson/webhooks/telegram -H "X-Telegram-Bot-Api-Secret-Token: $REDACTED" -d '{"update_id":1,"message":{"chat":{"id":1,"type":"private"},"text":"oi"}}'` | 200                                                          | 10s       | [EXISTE]                                 | —                            |
| Rota legacy Evolution   | `curl -X POST https://api.visualsmartflow.com.br/api/dr-anderson/webhook -d '{}'`                                                                                                                                                            | 410 Gone                                                     | 5s        | [EXISTE]                                 | OK se 410                    |
| Bot existe              | `curl https://api.telegram.org/bot$TELEGRAM_DEFAULT_BOT_TOKEN/getMe`                                                                                                                                                                         | `{"ok": true, ...}`                                          | 10s       | [NOVO D05k]                              | Token errado                 |
| Webhook registrado      | `curl https://api.telegram.org/bot$TELEGRAM_DEFAULT_BOT_TOKEN/getWebhookInfo`                                                                                                                                                                | url = `https://api.visualsmartflow.com.br/webhooks/telegram` | 10s       | [NOVO D05k]                              | `setWebhook` não foi chamado |

> **D05k:** webhooks Evolution foram descontinuados. Endpoints
> `/api/dr-anderson/webhook` e `/api/tenant/webhook` retornam **410 Gone**. Use
> `/api/dr-anderson/webhooks/telegram` e `/api/tenant/webhooks/telegram`. Ver
> `docs/TELEGRAM_SETUP.md` para setup completo.
...
> **Correção M22.1:** removido teste de `curl /metrics` no backend. Endpoint não
> existe no código.
...
> **Correção M22.1:** removido teste direto contra Prometheus/Grafana.
> Substituído por teste ao script `healthcheck.sh` que escreve em
...
- ⚠️ Falhar mas com causa conhecida (ex: `/api/health` 404, Prometheus não
  deployado, CSP unsafe-inline)
