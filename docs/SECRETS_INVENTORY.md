...
reconstruído a partir de `.env.example`, `.env.production.example`, `config.py`,
`docker-compose.prod.yml`, `docker-compose.staging.yml`, `scripts/`,
`services/`, `security_config.py`.

**REGRA:** Nenhum valor de secret é mostrado. Apenas nomes, obrigatoriedade e
procedência.
...
**Tamanho mínimo:** ≥32 chars para cada um. Gerar com
...
## 6. Telegram Bot API (D05k — substitui WhatsApp/Evolution)

| Variável                                       | Obrigatória?    | Origem verificada                                 | Onde configurar                     | Consequência se ausente                          |
| REDACTED | --------------- | REDACTED | REDACTED | REDACTED |
| `TELEGRAM_DEFAULT_BOT_TOKEN`                   | 🟡              | `.env.example`, `services/telegram_service.py:33` | env (provisionado via @BotFather)   | `notify_admin_*` silenciosamente não envia       |
| `TELEGRAM_ADMIN_CHAT_ID`                       | 🟡              | `.env.example`, `services/telegram_service.py:88` | env (capturado via `/start` no bot) | `notify_admin_*` silenciosamente não envia       |
| `REDACTED`          | 🟡              | `routes/dr_anderson_webhook.py:127`               | env                                 | Webhook Telegram dedicado retorna 401            |
| `ConfiguracaoIA.telegram_bot_token`            | 🟢 (per-tenant) | DB                                                | painel admin / SQL                  | Multi-tenant webhook retorna 401                 |
| `ConfiguracaoIA.telegram_webhook_secret`       | 🟢 (per-tenant) | DB                                                | painel admin / SQL                  | Multi-tenant webhook retorna 401                 |
| `ConfiguracaoIA.telegram_notification_chat_id` | 🟢 (per-tenant) | DB                                                | painel admin / SQL                  | Mensagens ao tenant silenciosamente não enviadas |

> **D05k:** `WHATSAPP_API_URL`, `WHATSAPP_API_KEY`, `WHATSAPP_INSTANCE_NAME`,
> `WHATSAPP_ADMIN_PHONE`, `WHATSAPP_TOKEN`, `EVOLUTION_API_URL`,
> `EVOLUTION_API_KEY`, `EVOLUTION_WEBHOOK_SECRET` foram **removidas** do código
> e do startup check. Ver `docs/TELEGRAM_SETUP.md` para setup completo.
...
> **Inconsistência detectada:** `.env.example` usa
> `SMTP_SERVER`/`SMTP_USERNAME`/`SMTP_USE_SSL`/`EMAIL_FROM`, enquanto
> `.env.production.example` usa `SMTP_HOST`/`SMTP_USER`/`SMTP_TLS`/`SMTP_FROM`.
> Verificar qual o código realmente lê antes do deploy.
...
> **Nota:** os 9 secrets de CI/CD não foram verificados diretamente nos arquivos
> `.yml` (não foram lidos nesta missão), mas referenciados como padrão em
> MISSÃO 20.
...
**Validador:** executar `python3 scripts/validate_env.py` antes de qualquer
deploy. Falha aborta deploy.

**Nota operacional:** inconsistência entre `.env.example` e
`.env.production.example` para SMTP (variáveis com nomes diferentes) deve ser
resolvida pelo time antes do deploy.
