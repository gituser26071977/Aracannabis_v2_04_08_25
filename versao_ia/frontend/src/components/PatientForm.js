...
const PatientForm = ({ onSave, initialData = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Converter data_nascimento para ISO antes de enviar
      const dataToSend = {
        ...formData,
        data_nascimento: formatDateToISO(formData.data_nascimento)
      };
      
      if (initialData?.id) {
        // Atualizar paciente existente
        console.log('Atualizando paciente existente com ID:', initialData.id);
        result = await pacientesService.atualizar(initialData.id, dataToSend);
      } else {
        // Criar novo paciente
        console.log('Criando novo paciente');
        result = await pacientesService.criar(dataToSend);
      }
