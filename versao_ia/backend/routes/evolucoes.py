...
def listar_evolucoes(paciente_id):
    ...
    evolucoes_data = [e.to_dict() for e in evolucoes]

    # Buscar exames (usando mesma lógica do endpoint de exames)
    from models import Exame
    exames_query = Exame.query.filter_by(paciente_id=paciente_id)
    if termo_busca:
    ...
    exames = exames_query.order_by(Exame.data_exame.desc()).all()
    exames_data = [ex.to_dict() for ex in exames]
