...
const DosageManager = ({ patientId }) => {
  ...
  // Aplicar sugestão do assistente ao formulário manual
        ...
        <>
          ...
          <TabPanel value={tabValue} index={1}>
            <Paper elevation={3} sx={{ p: 3 }}>
              ...
              {/* Formulário de novo produto e edição */}
              {showProdutoForm && (
                <ProductForm
                  initialData={produtoFormInicial}
                  onSubmit={async (produtoData) => {
                    try {
                      if (produtoFormInicial && produtoFormInicial.id) {
                        // Modo de edição
                        const response = await produtosService.atualizar(produtoFormInicial.id, produtoData);
                        setProdutos(prev => prev.map(p => p.id === produtoFormInicial.id ? response.produto : p));
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                              <IconButton
                                ...
                                onClick={() => {
                                  ...
                                  setShowProdutoForm(true);
                                  // Scroll to form
                                  const element = document.getElementById('product-form-anchor');
