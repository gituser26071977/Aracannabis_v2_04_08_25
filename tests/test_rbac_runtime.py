...
def app_with_rbac():
    ...
    app.config["JWT_SECRET_KEY"] = "test-rbac-runtime-secret-key-32bytes"
    app.config["SECRET_KEY"] = "test-rbac-runtime-secret-key-32bytes"
...
class TestPrescricoesRBAC:
    ...
    def test_physician_can_create_prescricao(self, client, app_with_rbac):
    ...
    def test_secretary_cannot_create_prescricao(self, client, app_with_rbac):
    ...
    def test_auxiliar_legacy_cannot_create_prescricao(self, client, app_with_rbac):
...
class TestEvolucoesRBAC:
    ...
    def test_physician_can_register_evolucao(self, client, app_with_rbac):
        token = _make_token(app_with_rbac, 3)
        res = client.post(
    ...
    def test_secretary_cannot_register_evolucao(self, client, app_with_rbac):
        token = _make_token(app_with_rbac, 4)
        res = client.post(
            "/api/evolucoes/paciente/1",
            headers={"Authorization": f"Bearer {token}"},
...
class TestConsultasRBAC:
    ...
    def test_secretary_can_schedule_consulta(self, client, app_with_rbac):
        token = _make_token(app_with_rbac, 4)
        res = client.post(
            "/api/consultas/",
            headers={"Authorization": f"Bearer {token}"},
...
class TestConfigIARBAC:
    ...
    def test_secretary_cannot_view_config_ia(self, client, app_with_rbac):
        token = _make_token(app_with_rbac, 4)
        res = client.get(
            "/api/config-ia",
            headers={"Authorization": f"Bearer {token}"},
