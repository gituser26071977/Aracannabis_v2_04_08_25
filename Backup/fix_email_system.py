...
def backup_email_service():
    ...
    try:
        shutil.copy('services/email_service.py', 'services/email_service_backup.py')
        print("✅ Backup do email_service.py criado")
        return True
    except Exception as e:
        print(f"❌ Erro ao fazer backup: {e}")
        return False

def create_development_email_service():
    """Criar serviço de email para desenvolvimento"""
...
'''
    ...
    try:
        with open('services/email_service.py', 'w', encoding='utf-8') as f:
            f.write(email_service_content)
        print("✅ Serviço de email atualizado com modo desenvolvimento")
        return True
    except Exception as e:
...
def update_env_file():
    """Atualizar arquivo .env com modo desenvolvimento"""
    try:
        # Ler arquivo .env atual
        with open('.env', 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Adicionar configuração de modo desenvolvimento se não existir
        if 'EMAIL_DEVELOPMENT_MODE' not in content:
            content += '\n# Modo desenvolvimento para emails\nEMAIL_DEVELOPMENT_MODE=True\n'
            
            with open('.env', 'w', encoding='utf-8') as f:
                f.write(content)
            
            print("✅ Arquivo .env atualizado com EMAIL_DEVELOPMENT_MODE=True")
        else:
            print("✅ EMAIL_DEVELOPMENT_MODE já configurado no .env")
...
def main():
    print("🔧 Corrigindo sistema de email...")
    
    # Fazer backup
    backup_success = backup_email_service()
    
    # Criar novo serviço
    service_success = create_development_email_service()
